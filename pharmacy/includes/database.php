...
function pharmacy_clear_tgp_inventory(PDO $pdo): void
{
    ...
    $meta->execute(['clear_tgp_inventory_v1']);
    if ($meta->fetchColumn()) {
        return;
    }

    $pharmaciesExist = $pdo->query("SHOW TABLES LIKE 'pharmacies'")->fetch();
    $medicinesExist = $pdo->query("SHOW TABLES LIKE 'medicines'")->fetch();
    if ($pharmaciesExist && $medicinesExist) {
        $ids = $pdo->query("
        ...
        if ($ids !== []) {
            ...
            $delete = $pdo->prepare('DELETE FROM medicines WHERE pharmacy_id IN (' . $placeholders . ')');
            $delete->execute(array_values(array_map('strval', $ids)));
        }
    }

    $mark = $pdo->prepare('INSERT IGNORE INTO app_meta (meta_key, meta_value) VALUES (?, ?)');
    $mark->execute(['clear_tgp_inventory_v1', date('c')]);
}
...
function pharmacy_seed_tgp_inventory(PDO $pdo): void
{
    ...
    $rows = [
        [
            ...
            'ingredients' => "Active: Amoxicillin\nInactive: Starch, povidone, magnesium stearate",
        ...
        [
            ...
            'description' => 'Used for temporary relief of pain and fever.',
            'ingredients' => "Active: Paracetamol\nInactive: Starch, povidone, magnesium stearate",
            'batch_number' => 'PCM-2602',
