...
function admin_database_hidden_columns(): array
{
    return ['password_hash', 'content', 'data'];
}

function admin_database_tables(): array
{
    try {
        $pdo = caps_db();
        $rows = $pdo->query(
        ...
        foreach ($rows as $row) {
            ...
            $count = (int) $pdo->query('SELECT COUNT(*) FROM `' . $name . '`')->fetchColumn();
            $tables[] = [
                'name' => $name,
                'rows' => $count,
            ];
...
function admin_database_table_view(string $table, int $page = 1, int $perPage = 25): array
{
    $empty = ['table' => '', 'columns' => [], 'rows' => [], 'total' => 0, 'page' => 1, 'pages' => 1];
    ...
    try {
        ...
        $columnRows = $pdo->query('SHOW COLUMNS FROM `' . $table . '`')->fetchAll();
        $hidden = admin_database_hidden_columns();
        $columns = [];
        ...
        $select = $columns === [] ? '*' : implode(', ', array_map(static fn (string $col): string => '`' . $col . '`', $columns));
        $stmt = $pdo->query('SELECT ' . $select . ' FROM `' . $table . '` LIMIT ' . (int) $perPage . ' OFFSET ' . (int) $offset);
...
function admin_database_cell(mixed $value): string
{
    if ($value === null) {
        return 'NULL';
    }
