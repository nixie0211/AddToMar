(function () {
  ...
  function setHiddenCoords(lat, lng) {
    const latInput = document.getElementById('register-latitude');
    const lngInput = document.getElementById('register-longitude');
    if (latInput) latInput.value = String(lat);
    if (lngInput) lngInput.value = String(lng);
  }

  function setLocateLoading(isLoading) {
    const btn = document.getElementById('register-map-locate-btn');
    if (!btn) return;
  ...
  function setMapStatus(message, type) {
    const statusEl = document.getElementById('register-map-status');
    if (!statusEl) return;
  ...
  function hideSearchResults() {
    const list = document.getElementById('register-map-search-results');
    if (list) {
      list.hidden = true;
      list.innerHTML = '';
    }
  ...
  function showSearchResults(results) {
    ...
    list.querySelectorAll('.register-map-search-item').forEach((button) => {
      button.addEventListener('click', () => {
        ...
        hideSearchResults();
        const searchInput = document.getElementById('register-map-search');
  ...
  function bindToolbar() {
    ...
    const locateBtn = document.getElementById('register-map-locate-btn');
    const searchInput = document.getElementById('register-map-search');
    const searchBtn = document.getElementById('register-map-search-btn');
    ...
    document.addEventListener('click', (event) => {
      if (!event.target.closest('.register-map-search-wrap')) {
        hideSearchResults();
      }
    });
  }
  ...
  window.initRegisterMap = function initRegisterMap() {
    ...
    if (!map) {
      ...
      const savedLat = parseFloat(document.getElementById('register-latitude')?.value || '');
      const savedLng = parseFloat(document.getElementById('register-longitude')?.value || '');
